...
def train(model, epochs, trainloader, valloader, device, optimizer, loss_fn):
    ...
    for epoch in range(epochs):
        ...
        # Early stopping
        print("Current validation loss: {:.2f}".format(np.mean(val_losses)))
        all_val_losses.append(np.mean(val_losses))
        all_epochs.append(epoch + 1)
        x = np.array(all_val_losses[-4:])
        dx = np.abs(np.diff(x))
        # train at least 10 epochs
        if np.all(dx < 0.01) and len(all_val_losses) > 10:
            print("Early stopping!")
            break

    print("Training finished. Time >{} minutes.".format((time.time()-start) // 60))
    plt.title('Validation loss')
    plt.plot(all_epochs, all_val_losses)
    plt.legend(loc = 'lower right')
    plt.ylabel('Validation loss')
    plt.xlabel('Epochs')
    plt.savefig(fpath+'loss_inception_nonweighted.png')
    plt.show()
