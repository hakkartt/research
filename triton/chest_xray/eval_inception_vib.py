...
for beta in betas:
    ...
    for e in epss:
        spsas[e].append(
                evaluate(
                        model, testloader, device,
                        fname_vib, adv='SPSA', n_iter=5, eps=e)
                )
#    evaluate(model, testloader, device, fname_vib, adv='CW')

fig = plt.figure(figsize = (12, 12))
plt.title("Accuracies w.r.t. betas for different adversaries")
plt.plot(betas, noattacks, label='no attack')
plt.plot(betas, fgms, label='FGSM')
for k, v in pgds.items():
    plt.plot(betas, v, label='PGD, n_iter={}'.format(k))
for k, v in spsas.items():
    plt.plot(betas, v, label='SPSA, eps={}'.format(k))
plt.legend(loc='upper right')
plt.ylabel('Accuracy')
plt.xlabel('Beta')
plt.xscale('log')
plt.savefig(fpath+'inception_beta_comparison.png')
